...
CREATE SEQUENCE BOOK_SEQ
    ...
    CACHE 20
    ORDER;
-- 옵션의 적는 순서는 관계가 없다. 자유롭게 작성자 마음대로 가능
...
INSERT INTO BOOK_T(BOOK_ID, BOOK_NAME, PUBLISHER, PRICE) VALUES(BOOK_SEQ.NEXTVAL, '역도의 단계별 기술', '굿스포츠' , 6000);
...
-- 230708-8 10     올림픽 챔피언
-- 230709-9 10     올림픽 챔피언
SELECT O.ORDER_ID   AS 주민번호 ,
       B.BOOK_ID    AS 책번호,
       B.BOOK_NAME  AS 책이름
  FROM BOOK_T B INNER JOIN ORDER_T O
    ON B.BOOK_ID = O.BOOK_ID
 WHERE O.ORDER_DT BETWEEN TO_DATE('20/07/05') AND TO_DATE('20/07/09') --ORDER_DT TO_DATE 처리할 필요가없는 이유가 이미 입력시 TO_DATE 처리를 하였기 떄문이다.
...
-- 12. 가장 최근(MAX)에 구매한 고객의 이름, 책이름, 주문일자를 조회하시오.
-- 고객명  책이름            주문일자
...
-- 15. '김연아'가 구매한 도서수를 조회하시오. (구매한 횟수면 GROUP BY절 사용)
-- 고객명  구매도서수
-- 김연아  2
SELECT C.CUST_NAME       AS 고객명,
...
SELECT C.CUST_NAME               AS 고객명,
       COUNT(DISTINCT O.BOOK_ID) AS 구매도서수 --주문을 몇번 했는가
  FROM CUSTOMER_T C INNER JOIN  ORDER_T O
    ON C.CUST_ID = O.CUST_ID
 WHERE C.CUST_NAME ='김연아'
 GROUP BY C.CUST_ID, C.CUST_NAME;
...
SELECT B.PUBLISHER AS 출판사,
  ...
  FROM BOOK_T B LEFT OUTER JOIN ORDER_T O  -- 출판사 기준 이유 출판사 0개의 내역도 조회하기 위해서
    ON B.BOOK_ID = O.BOOK_ID
