/*
    ROWID
    1. 오라클에서 제공하는 가상 칼럼(PSEUDO COLUMN)슈도 칼럼 이다.
...
/*
    ...
    4. 인덱스가 많으면 데이터의 삽입/수정/삭제 시 인덱스도 함께 갱신해야 하기 떄문에 전체적인 성능이 떨어질 수 있다.
*/
...
-- 인덱스를 안 타는 조회 (인덱스 칼럽을 그대로 사용하지 않고 함수를 적용해서 사용하면 인덱스를 타지 않는다.)
SELECT *
  FROM EMPLOYEES
 WHERE TO_CHAR(EMPLOYEE_ID) = '150';

-- 인덱스를 적을시 = 기준으로 왼쪽을 건들지 않고 오른쪽을 수정 하는 방식을 사용하라
-- 이유 : 그에 따른 성능 저하 현상이 발생할수 있기 떄문이다.
...
-- 인덱스 정보가 저장되어 있는 데이터사전(미리 만들어진 테이블)
-- DBA_INDEXES, USER_INDEXES, ALL_INDEXES
SELECT * FROM USER_INDEXES;     -- HR 계정에 저장된 인덱스
SELECT * FROM ALL_INDEXES;

-- 인덱스가 설정된 칼럼 정보가 저장되어 있는 데이터 사전
-- DBA_IND_COLUMNS, USER_IND_COLUMNS, ALL_IND_COLUMNS,
SELECT * FROM USER_IND_COLUMNS;  -- 어느정도에 위치에 있는 칼럼을 명확히 확인 가능

SELECT * FROM USER_IND_COLUMNS WHERE TABLE_NAME = 'EMPLOYEES'; 
-- 해당 내용조회시 대소문자 일치해야된다. 대소문자 일치 하지 않으면 조회 X 불가능하다.
