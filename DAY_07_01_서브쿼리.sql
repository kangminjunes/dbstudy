...
/* FROM 문의 서브쿼리 */
-- 사용 주된 이유 : 실행순서 떄문이다.
-- FROM절이 실행순서가 1번 가장 먼저 실행하기 위해서 사용한다.
...
-- 3. 21~30번쨰로 입사한 사원 조회하기 HIRE_DATE
SELECT HD, EMPLOYEE_ID
  FROM (SELECT ROW_NUMBER() OVER(ORDER BY HIRE_DATE DESC) AS HD, EMPLOYEE_ID
          FROM EMPLOYEES)
...
-- 서브쿼리와 메인쿼리가 상관없는것이 비상관커리 이다.
-- 메인쿼리 실행시 서브 쿼리 영향 없고 반대로 실행하여도 없기 떄문이다.
