/*
    ...
    * 문법이 어려운것이 아닌 내부,외부 조인 판단하는것이 어렵다.
...
해당을 1:M 관계로 가능 회원 아이디 a,b,c를 가지고 묶는것을 가능
...
/*
    ...
    1. 드라이브 테이블
        ...
        3) 일반적으로 데이터의 갯수가 적다.
        4) PK를 조인 조건으로 사용하기 떄문에 인덱스(INDEX) 사용이 가능하다. (빠르다는 의미)
    2. 드리븐테이블
        1) 1:M 관계에서 M에 해당하는 테이블
        2) 일반적으로 데이터의 갯수가 많다.
        3) FK를 조인 조건으로 사용하기 떄문에 인덱스(INDEX) 사용이 불가능하다.(느리다는 의미)
...
SELECT EMPLOYEE_ID ,
       FIRST_NAME ,
       LAST_NAME ,
       D.DEPARTMENT_ID, -- 2개의 테이블에 모두 있는칼럼은 반드시 테이블(오너)을 명시해야한다.
       DEPARTMENT_NAME
