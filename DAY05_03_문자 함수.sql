...
--       CONCAT(CONCAT(A, B), C) : 인수 3개이상은 CONCAT 함수 여러 개로 해결한다.
SELECT *
  FROM EMPLOYEES
...
-- 5. 일부만 반환하기
SELECT SUBSTR(PHONE_NUMBER, 1, 3), -- 전화번호 1번쨰 글자부터 3글자를 반환
       SUBSTR(PHONE_NUMBER, 5)     -- 전화번호 5번쨰 글자부터 끝까지  반환
  FROM EMPLOYEES;
...
-- 9. 공백 제거 (왼쪽 오른쪽 가능, 중간에 있는거는 제거 불가능)
SELECT '[' || LTRIM('     HELLO     WORLD     ') || ']', --   왼쪽 공백 제거
       '[' || RTRIM('     HELLO     WORLD     ') || ']', -- 오른쪽 공백 제거
       '[' ||  TRIM('     HELLO     WORLD     ') || ']'  -- 왼쪽/오른족 공백 제거
  FROM DUAL;
  
-- 실습.
-- 1. 사원 테이블의 JOB_ID에서 밑줄(_) 앞/뒷 부분만 분리조회하기
-- 예시) IT_PROG -> IT / PROG
--       글자수   : 7
--       밑줄위치 : 3
--       밑줄 앞 글자수 : 2(밑줄위치 -1)
...
-- 2. FIRST_NAME과 LAST_NAME 연결해서 모두 대문자로 바꾼 FULL_NAME 조회하기
-- 예시) FITST_NAME : steven
--        LAST_NAME : KING
