...
/*
    ...
    5. 테이블 이름 : ALTER TALBE 테이블명 RENAME TO 신규 테이블명
    6. PK/FK 제약조건
        1) PK
            (1) 추가
...
-- 5. 고객 테이블 GRADE 칼럼을 추가하시오.('VIP' ,'GOLD', 'SIVER', 'BRONZE'중 하나의 값을 가지도록 한다.
ALTER TABLE CUSTOMER_TBL ADD GRADE VARCHAR2(6 BYTE) CHECK(GRADE = 'VIP' OR GRADE ='GOLD' OR GRADE = 'SIVER' OR GRADE = 'BRONZE');
ALTER TABLE CUSTOMER_TBL ADD GRADE VARCHAR2(6 BYTE) CHECK(GRADE = IN('VIP' ,'GOLD', 'SIVER', 'BRONZE')); 
...
-- 7. 고객 테이블의 연락처(CUST_NAME) 칼럼을 필수 칼럼으로 변경하시오.
ALTER TABLE CUSTOMER_TBL MODIFY CUST_NAME VARCHAR2(30 BYTE) NOT NULL ;

-- 8. 고객 테이블의 고객명(CUST_NAME) 칼럼의 필수 제약조건을 없애시오.
ALTER TABLE CUST_NAME MODIFY CUST_NAME VARCHAR2(30 BYTE) NULL; --반드시 NULL을 명시해야 한다.
